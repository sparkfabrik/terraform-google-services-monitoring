...
locals {
  ...
  # Cost thresholds are only materialised when the service is on and at least
  # one model carries a price: an alert on an expression that is constantly
  # vector(0) would never fire and would only add noise to the plan.
  vertex_ai_cost_alerts = var.vertex_ai.enabled && length(local.vertex_ai_cost_terms) > 0 ? {
