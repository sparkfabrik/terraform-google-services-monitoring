...
- `vertex_ai.alerts.error_rate.min_invocations` (default 20): a floor on the denominator of the error-rate ratio, so a model is evaluated only once it has taken enough calls in the window for a percentage to mean something.
...
- `vertex_ai.pricing`: a hand-maintained price table shipped as a module default, keyed by model, token type and endpoint class, overridable per consumer, with `pricing_verified_on` shown on the dashboard. Cost is estimated as tokens multiplied by list price, in USD, because Vertex AI publishes no spend metric to Cloud Monitoring. The table must be reviewed periodically, and the README gives the Cloud Billing Catalog query that checks it.
- The estimated cost is documented as an upper bound, on the dashboard, in the alert documentation and in the README. Cloud Monitoring reports prompt tokens served from Gemini's implicit context cache under the same `input` type as uncached ones, and Google bills them at a tenth of the input price, so the estimate charges them at the full rate and reads high whenever caching is working.
