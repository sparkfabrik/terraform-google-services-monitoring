...
- `exclude_patterns` (optional, default `[]`) on the Typesense `log_check` block: a list of substrings excluded from the log-match alert filter via `AND NOT (jsonPayload.message:"<pattern>" OR textPayload:"<pattern>" ...)`; matching uses the case-insensitive Cloud Logging substring operator. The flood check and the dashboard error-log metric keep counting excluded entries. Patterns that are empty after trimming or contain a double quote, a backslash, or a control character are rejected at plan time.
