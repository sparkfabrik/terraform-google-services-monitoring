...
- `vertex_ai.alerts.cost.notification_channels` and `vertex_ai.alerts.cost.notification_prompts`: routing declared once for the whole cost family, inherited by every threshold and overridable per threshold. Resolution runs threshold, then cost family, then service, then the module root, matching the Typesense services. An enabled alert that resolves to no channel is rejected at plan time; `notification_enabled = false` is how a check stays silent on purpose.
