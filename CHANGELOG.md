...
- `log_check.exclude_patterns` validation now also rejects patterns that are whitespace-only, contain a backslash, or contain a newline: a trailing backslash escapes the closing quote in the Cloud Logging filter grammar and a whitespace-only pattern silences every log line.
