...
6. **Strengthened pattern validation.** The parent change's validation (non-empty, no `"`) does not actually make verbatim interpolation safe: a trailing backslash escapes the closing quote in the Cloud Logging filter grammar, and a whitespace-only pattern silences every log line. The validation now rejects patterns that are empty after trimming or contain `"`, `\`, or a newline. Constant error message (dynamic messages need Terraform >= 1.9; the module floor is 1.5). Only configurations that already produced broken or degenerate filters are rejected, so this is a fix, not a breaking change.
